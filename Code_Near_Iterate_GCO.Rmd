...
fishnet.barley<-subset(fishnet, fishnet$mean_barle > 0 ) #fert > 0 
...
fishnet.cassava<-subset(fishnet, fishnet$mean_cassa > 0 ) 
...
fishnet.groundnut<-subset(fishnet, fishnet$mean_groun > 0 ) 
...
fishnet.maize<-subset(fishnet, fishnet$mean_maize > 0 ) 
...
fishnet.potato<-subset(fishnet, fishnet$mean_potat > 0 ) 
...
for(i in 1:N){
  ...
  distances <- spDistsN1(xy,xy[random.point,], longlat = FALSE)
  yield$rescale_ND<-distances/(1000*1000)
  Potato.near<-yield %>% select(mean_potat,rescale_ND)
  Potato.near<-Potato.near %>% filter(mean_potat > 0, na.rm=TRUE)
  Potato.near<-Potato.near %>% filter(rescale_ND > 0, na.rm=TRUE)
  Potato.near$logPotato<-log10(Potato.near$mean_potat)
  
  # fit yield ~ distance model
  Potato.ols <- gls(logPotato ~ rescale_ND, data = Potato.near)
  
  # associate it to the point
  output[i,] <- c(coef(Potato.ols)[2], xy[random.point,], summary(Potato.ols)$tTable[2,4] )
}

#Plot it!
ggplot(output, aes(x=x, y=y, color=slope)) +geom_point() +theme_justin 

#write.csv(output, file="Near_Null/Potato_Null_10000.csv")
...
fishnet.rapeseed<-subset(fishnet, fishnet$mean_rapes > 0 ) 
...
fishnet.rice<-subset(fishnet, fishnet$mean_rice_ > 0 ) 
...
fishnet.rye<-subset(fishnet, fishnet$mean_rye_Y > 0 ) 
...
fishnet.sorghum<-subset(fishnet, fishnet$mean_sorgh > 0 ) 
...
fishnet.soy<-subset(fishnet, fishnet$mean_soybe > 0 ) 
xy <- coordinates(fishnet.soy)
yield <- fishnet.soy@data
...
fishnet.sunf<-subset(fishnet, fishnet$mean_sunfl > 0 ) 
xy <- coordinates(fishnet.sunfl)
yield <- fishnet.sunfl@data
# the loop
...
fishnet.wheat<-subset(fishnet, fishnet$mean_wheat > 0 ) 
