...
rf<-na.omit(rf.potato)
rf<-log10(rf+1)
summary(rf)
sapply(rf.potato, "class")

# create a list of 60% of the rows in the original dataset we can use for training
validation_index <- createDataPartition(rf$mean_potat, p=0.60, list=FALSE)
# select 20% of the data for validation
validation <- rf[-validation_index,]
# use the remaining 60% of data to training and testing the models
dataset <- rf[validation_index,]     
...
# Random Forest
set.seed(7)
fit.rf.potato <- train(mean_potat~., data=dataset, method="rf", metric=metric, trControl=control) #remove # to run again
print(fit.rf.potato) #print results
#write.csv(fit.rf.potato$results, "Models/potato_rfFIT.csv")

stopCluster(cl)


potato.varIMP<-(varImp(fit.rf.potato)) #identify variable importance
plot(potato.varIMP)
#write.csv(potato.varIMP$importance, file = "Models/potato_varIMP.csv")
# Model Performance
predictions <- predict(fit.rf.potato, validation) #predict 
pred.error<-RMSE(predictions,validation$mean_potat) #get the Root Mean Standard Error of predicted values
exp(pred.error-1) #put in yield values

modelerror <- exp(predict(fit.rf.potato, rf )-1) #predict on whole dataset
#summary(modelerror) #make sure it worked honeyyyy

#write.csv(modelerror,file="Models/Predictions/potato_predictions.csv") # export for mapping
...
# create a list of 60% of the rows in the original dataset we can use for training
validation_index <- createDataPartition(rf$potato_HgHa, p=0.60, list=FALSE)
# select 20% of the data for validation
validation <- rf[-validation_index,]
# use the remaining 60% of data to training and testing the models
dataset <- rf[validation_index,]     
...
# Random Forest
set.seed(7)
fit.rf.potato <- train(potato_HgHa~., data=dataset, method="rf", metric=metric, trControl=control) #remove # to run again
print(fit.rf.potato) #print results
#write.csv(fit.rf.potato$results, "Models/potato_rfFIT.csv")
...
# Model Performance
predictions <- predict(fit.rf.potato, validation) #predict 
pred.error<-RMSE(predictions,validation$potato_HgHa) #get the Root Mean Standard Error of predicted values
exp(pred.error-1) #put in yield values
