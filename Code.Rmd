...
### Climate: Still working on with Google Earth Engine
...
mapping<-read.csv("mapping_GCOfishnet.csv", header=T)
...
ggplot(barley.m, aes(x=barley.m$barley_GCO, y=barley.m$value, color=barley.m$barley_GCO)) + 
  ...
  geom_boxplot(color="black", fill="transparent", lwd=1)+
...
ggplot(rice.m, aes(x=rice.m$rice_GCO, y=rice.m$value, color=rice.m$rice_GCO)) + geom_jitter(alpha=0.4) + 
  geom_boxplot(color="black", fill="transparent", lwd=1)+
  scale_color_brewer(palette = "Dark2") + 
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
...
```{r, Analysis: Random Forest Trial,eval=FALSE}
```

Hotspot analyses (Getis-Ord G*) were conducted in ArcMap, ESRI to identify where yields are significantly higher with regards to the global distribution of all neighbors. This tool works by looking at each feature within the context of neighboring features. A feature with a high value is interesting but may not be a statistically significant hot spot. 

To be a statistically significant hot spot, a feature will have a high value and be surrounded by other features with high values as well. The local sum for a feature and its neighbors is compared proportionally to the sum of all features; when the local sum is very different from the expected local sum, and when that difference is too large to be the result of random chance, a statistically significant z-score results. False Discovery Rate (FDR) was applied and statistical significance was adjusted to account for multiple testing and spatial dependency. 

Assumptions: They are flexible depending on the data, one of the most important things is the spatial conceptualization (see Inverse Distance below) where we define how the model will consider spatial neighbors. In theory everything is normally distributed and random; however this method is quite robust to data not following this structure (as is common with spatial data, thus why it is so popular)

An Inverse distance weight spatial conceptualization was used where closer values to the start pixel are assumed to be more similar (as an extension of Tobler's law), likewise the crop yields are continuous across (ranging from  0 - local/global maxima). A distance band was caluclated so that each pixel includes at least 1 neighbor (mean of 2133.641 km)

```{r, Hotspot Analysis - Conducted in ArcMap, ESRI}
