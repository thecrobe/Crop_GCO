...
hist(rice.filter$rice_HgHa)
qqnorm(rice.filter$rice_HgHa)
dim(rice) - dim(sorg.filter) # Num Outliers Removed
summary(rice)
summary(rice.filter)
rice<-rice.filter
...
#Permutational Spearman Correlartion
near.barley<-perm.relation(x = log10(barley.near$NEAR_DIST),barley.near$mean_barle,method = "spearman")
...
ggplot(barley.near, aes(x=log10(NEAR_DIST), y=log10(barley.near$mean_barle))) +
  geom_point(alpha=0.2, color="#00A9B4") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
#Permutational Spearman Correlartion
near.cassava<-perm.relation(x = log10(cassava.near$NEAR_DIST),cassava.near$mean_cassa,method = "spearman")
...
#Permutational Spearman Correlartion
near.groundnut<-perm.relation(x = log10(groundnut.near$NEAR_DIST),groundnut.near$mean_groun,method = "spearman")
...
#Permutational Spearman Correlartion
near.rapeseed<-perm.relation(x = log10(rapeseed.near$NEAR_DIST),rapeseed.near$mean_rapes,method = "spearman")
...
#Permutational Spearman Correlartion
near.sorghum<-perm.relation(x = log10(sorghum.near$NEAR_DIST),sorghum.near$mean_sorgh,method = "spearman")
...
#Permutational Spearman Correlartion
near.wheat<-perm.relation(x = log10(wheat.near$NEAR_DIST),wheat.near$mean_wheat,method = "spearman")
...
write.csv(10^(modelerror) –1),file="Models/Predictions/barley_predictions.csv") # export for mapping
...
```{r,fig.align='center',message=FALSE, warning=FALSE, cache=TRUE, fig.cap="Heatmap of Rho coefficients for each variable by crop. The final version of this is found in the manuscript and is annotated with Bonforonni corrected significance"}
...
ggplot(vard.m, aes(x=vard.m$Crop, y=vard.m$variable, fill=vard.m$value)) +geom_tile() + theme_bw() +theme(axis.line = element_line(colour = "black"),
    ...
    panel.border = element_blank(),
    panel.background = element_blank()) + 
  ylab("Crop") +xlab("Variable") + labs(color='Rho') +scale_fill_gradient(low="white", high="blue") +theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
