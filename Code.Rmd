...
```{r message=TRUE, warning=FALSE, cache=T}


### Sorghum 

sorghum.near<-read.csv(file = "Near_Coded/Sorghum_near.csv", header=T)
sorghum.near.filter<-sorghum.near %>% filter(between(sorghum.near$mean_sorgh, quantile(mean_sorgh, .01), quantile(mean_sorgh, .99))) #remove top and bottom 1% 
sorghum<-sorghum.near.filter
