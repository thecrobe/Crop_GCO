...
=======
```{r: Introduction, echo=FALSE}
>>>>>>> be4a2319461f521d10bb11f889bc03eb91d7d8b6
...
# Barley 
=======
```{r: Boxplots, Merge Mapping,echo=FALSE}
mapping<-read.csv("Fishnets/mapping_GCOfishnet.csv", header=T, fileEncoding = "UTF-8-BOM")
yield<-read.csv("Fishnets/yield_fishnet.csv", header=T)
...
ggplot(barley.near, aes(x=log10(NEAR_DIST), y=log10(mean_barle))) +
  geom_point(alpha=0.1, color="#00A9B4") + geom_smooth(method = "glm",color="#DB3500") +
=======
...
<<<<<<< HEAD
```{r Spatial Regression - Near Analysis, fig.align='center',message=FALSE, warning=FALSE, cache=TRUE}

### Barley ###
barley.near<-read.csv(file="./Near_All/Barley_Near_Centroid.csv", header=T)
barley.merge<-merge(mapping,barley.near,by="Fishnet_ID")
barley.coord<-data.frame(cbind(barley.merge$FROM_X,barley.merge$FROM_Y))
# Spatial Correlation
barley.sp.corr<-modified.ttest(log10(barley.merge$NEAR_DIST), log10(barley.merge$mean_barle),coords = barley.coord, nclass = 8) #Queens Case
print(barley.sp.corr)
