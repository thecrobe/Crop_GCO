...
india.Perc<-sum(canada$groundnut_)/sum(groundnut.pivot$.) 
...
#Potato -
```{r, message=TRUE, warning=FALSE, cache=TRUE, fig.cap="Potato Near near Model, "}

#Select Data and Rescale Distance
near<-read.csv(file="./Near_All/Potato_Near_Centroid.csv", header=T)
near$rescale_ND <- near$NEAR_DIST/(1000*1000)
potato.near<-near %>% select(mean_potat,rescale_ND)
potato.near<-potato.near %>% filter(mean_potat > 0, na.rm=TRUE)
potato.near<-potato.near %>% filter(rescale_ND > 0, na.rm=TRUE)
potato.near$logRapeseed<-log10(potato.near$mean_potat)

#Plot
ggplot(potato.near, aes(x=rescale_ND, y=logRapeseed)) +geom_point(alpha=0.2, color="#FFAF02") + geom_smooth(method = "lm",color="#DB3500") +theme_justin +xlab("Distance From GCO (1000's km)") +ylab ("Log10 Yield")

###  Model - Potato 
# regular OLS no variance structure
potato.ols <- gls(logRapeseed ~ rescale_ND, data = potato.near)
# varFixed (variance changes linearly with X)
...
# compare all models by AIC
AIC(potato.ols, potato.fixed, potato.power, potato.ConstPower, potato.exp)
#varPower
summary(potato.exp)
```
