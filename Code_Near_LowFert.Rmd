...
fishnet<- readOGR(dsn= "Shapefiles/", layer="Fishnet_yield_NoAntarctica")
fertilizer<-read.csv(file="Fertilizer_Near/FertilizerNear.csv", header=T) #import fert
...
for(i in 1:N){
  ...
  barley.near.low<-metadata %>% dplyr::select(mean_barle,rescale_ND_barley)
...
for(i in 1:N){
  ...
  cassava.near.low<-metadata %>% dplyr::select(mean_cassa,rescale_ND_cassava)
...
for(i in 1:N){
  ...
  groundnut.near.low<-metadata %>% dplyr::select(mean_groun,rescale_ND_groundnut)
...
for(i in 1:N){
  ...
  maize.near.low<-metadata %>% dplyr::select(mean_maize,rescale_ND_maize)
...
```{r,  message=TRUE, warning=FALSE, cache=TRUE, message=FALSE,echo=FALSE}
#### readins and preprocess ######################
fishnet.low<-subset(fishnet.m, fishnet.m$Potato_FertMean > 0 ) #fert > 0 
fishnet.low<-subset(fishnet.m, fishnet.m$mean_potat > 0 ) # yield > 0 
dim(fishnet) - dim(fishnet.low) # pixels left
quantile(fishnet.low$Potato_FertMean,na.rm = TRUE) #identify quantiles
fishnet.low.potato<-subset(fishnet.m, fishnet.m$Potato_FertMean < 26.91) #subset below 50% of data

### Get Lat-Long Baby Girl 
xy <- coordinates(fishnet.low.potato)
metadata <- fishnet.low.potato@data
...
for(i in 1:N){
  ...
  # calculate distances from that point
  distances <- spDistsN1(xy,xy[random.point,], longlat = FALSE)
  metadata$rescale_ND_potato<-distances/(1000*1000)
  potato.near.low<-metadata %>% dplyr::select(mean_mille,rescale_ND_potato)
  potato.near.low<-potato.near.low %>% filter(mean_mille > 0, na.rm=TRUE)
  potato.near.low<-potato.near.low %>% filter(rescale_ND_potato > 0, na.rm=TRUE)
  potato.near.low$logPotato<-log10(potato.near.low$mean_mille)
  
  # fit yield ~ distance model
  potato.low.gls <- gls(logPotato ~ rescale_ND_potato, data = potato.near.low)
  
  # associate it to the point
  output.low[i,] <- c(coef(potato.low.gls)[2], xy[random.point,],    summary(potato.low.gls)$tTable[2,4] )
}
...
#write.csv(output.low,file="Low_Fert/potato_low50percNull.csv")
```
...
for(i in 1:N){
  ...
  rapeseed.near.low<-metadata %>% dplyr::select(mean_rapes,rescale_ND_rapeseed)
...
for(i in 1:N){
  ...
  rice.near.low<-metadata %>% dplyr::select(mean_rice_,rescale_ND_rice)
...
for(i in 1:N){
  ...
  rye.near.low<-metadata %>% dplyr::select(mean_groun,rescale_ND_rye)
...
for(i in 1:N){
  ...
  sorghum.near.low<-metadata %>% dplyr::select(mean_sorgh,rescale_ND_sorghum)
...
for(i in 1:N){
  ...
  soybean.near.low<-metadata %>% dplyr::select(mean_soybe,rescale_ND_soybean)
...
for(i in 1:N){
  ...
  sunflower.near.low<-metadata %>% dplyr::select(mean_sunfl,rescale_ND_sunflower)
...
for(i in 1:N){
  ...
  wheat.near.low<-metadata %>% dplyr::select(mean_wheat,rescale_ND_wheat)
