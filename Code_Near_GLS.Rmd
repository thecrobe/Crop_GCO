...
# varPower (variance changes as a power function with X)
# Does Not Run groundnut.power <- update(groundnut.ols, . ~ ., weights = varPower(form = ~rescale_ND))
# varExp (variance changes as an exponential function of x)
groundnut.exp <- update(groundnut.ols, . ~., weights = varExp(form = ~ rescale_ND)) # error
# varConstPower (constant plus a power function of X (useful if X includes 0))
# Does not convergegroundnut.ConstPower <- update(groundnut.ols, . ~., weights = varConstPower(form = ~ rescale_ND))
# compare all models by AIC
...
#OLS but chost varExp as AIC is close
summary(maize.exp)
```
#Millet -
...
# compare all models by AIC
AIC(sunflower.ols,  sunflower.ConstPower, sunflower.exp)
#varPower
